...
# Capture real-time frames and classify actions
def real_time_classification():
    ...
    while True:
        ...
        if len(frames) == 32:  # Increase the sequence length to 64
            frame_sequence = torch.stack(frames)
            frames = []  # Clear the frames list for the next sequence

            if len(frame_sequence) < 32:
                frame_sequence = torch.cat([frame_sequence, torch.zeros(32 - len(frame_sequence), 256)])
