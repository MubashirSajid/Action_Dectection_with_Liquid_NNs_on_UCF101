...
# Load model
class LTCWithActivations(LTC):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.activations = None

    def forward(self, input, hx=None, timespans=None):
        output, hx = super().forward(input, hx, timespans)
        self.activations = hx[0]  # Save the activations from the hidden state
        return output, hx

def load_model(model_path, input_size, wiring, num_classes):
    ltc_model = LTCWithActivations(input_size, wiring, batch_first=True)
...
# Real-time classification with activation map
def real_time_classification():
    ...
    activation_map = torch.zeros(128, 64)  # Initialize activation_map with a default value
    ...
    while True:
        ...
        if len(frames) == 64:
            ...
            with torch.no_grad():
                output, activations = model(frame_sequence)
                print(f"activations shape: {activations.shape}")  # Debug: Print shape of activations
            ...
            activation_map = activations.mean(dim=1).squeeze(0).cpu()  # Use correct dimension

        # Overlay activation map on the frame
        frame_with_activation = overlay_activation(frame, activation_map)

        # Display the action prediction on the frame
        cv2.putText(frame_with_activation, f"Predicted action: {action}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.imshow('Webcam', frame_with_activation)  # Display the frame with activation map in a window
